...
# 向后兼容：auto_upgrade=true → auto
if [ "$_MODE" = "none" ] || [ -z "$_MODE" ]; then
...
**If `UPDATE_MODE=auto`:** Skip AskUserQuestion. Log "Auto-upgrading gstack v{old} → v{new}..." and proceed directly to Step 2. If `./setup` fails during auto-upgrade, restore from backup (`.bak` directory) and warn the user: "Auto-upgrade failed — restored previous version. Run `/gstack-upgrade` manually to retry."
...
```bash
~/.claude/skills/gstack/bin/gstack-config set update_mode auto
```
...
Tell user the snooze duration: "Next reminder in 24h" (or 48h or 1 week, depending on level). Tip: "Set `update_mode: auto` in `~/.gstack/config.yaml` for automatic upgrades."

**If "Never ask again":**
```bash
~/.claude/skills/gstack/bin/gstack-config set update_mode none
```
