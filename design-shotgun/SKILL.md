...
version: 1.0.0
description: |
  Design shotgun: generate multiple HTML wireframe variants, open a comparison board,
...
# Ensure analytics directory exists for timeline-log
...
If `PROACTIVE_PROMPTED` is `no` AND `LAKE_INTRO` is `yes`: After lake intro is handled,
...
You are a design brainstorming partner. Generate multiple HTML wireframe variants, open them
...
"This is /design-shotgun — your visual brainstorming tool. I'll generate multiple HTML
wireframe directions, open them side-by-side in your browser, and you pick your favorite.
...
like how this looks," take a screenshot for reference and generate improvement wireframes
...
Write each variant to `$_DESIGN_DIR/variant-{letter}.html`.

Create comparison board HTML at `$_DESIGN_DIR/comparison-board.html`:
- Show all variants side-by-side in a grid or tabs
- Include variant letter, name, and brief description
- Each variant should be viewable full-size via link/iframe
- Clean, minimal styling for easy comparison
...
The comparison board is a static HTML file at `$_DESIGN_DIR/comparison-board.html`.
Collect feedback via AskUserQuestion:

> "Review the design variants in your browser. Which direction do you prefer?
>
> A) Variant A — [name]
> B) Variant B — [name]
> C) Variant C — [name]
> ...
> X) None of these — generate new directions
>
> Rate each variant 1-5:
> - A: _
> - B: _
> - C: _
>
> What specifically do you like/dislike about each?
> What should I change in the next round?"

If the user chooses X (new directions): go back to Step 3a with refined brief.

If the user selects a preferred variant: proceed to Step 5.

## Step 5: Feedback Confirmation

After receiving feedback via AskUserQuestion, output a clear summary confirming what was understood:
...
The calling skill reads `approved.json` and the approved variant HTML file.
...
1. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
   to `~/.gstack/projects/$SLUG/designs/`. This is enforced.
